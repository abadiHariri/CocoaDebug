...
//
//  Replaces _WKWebView+Swizzling.m.
//  Swizzles WKWebView and WKUserContentController to intercept
//  console.log/error/warn/debug/info, network (XMLHttpRequest + fetch),
//  and script-message-handler registrations.
//
...
class _WKScriptMessageProxy: NSObject, WKScriptMessageHandler {
...
@objc class _WKWebViewSwizzling: NSObject {

    private static var swizzled = false

    /// Call once (e.g. from `CocoaDebug.enable()`) to install all swizzles.
    /// Idempotent; respects the `enableWKWebViewMonitoring_CocoaDebug` default.
    @objc static func enableIfNeeded() {
        guard !swizzled else { return }
        guard UserDefaults.standard.bool(forKey: "enableWKWebViewMonitoring_CocoaDebug") else { return }
        swizzled = true
        performSwizzling()
    }
...
extension WKUserContentController {
    ...
    @objc func replaced_add(_ handler: WKScriptMessageHandler, name: String) {
        // Don't wrap CocoaDebug's own handlers (WKWebView registers itself)
        if handler is _WKWebViewScriptHandler || handler is _WKScriptMessageProxy {
            replaced_add(handler, name: name) // calls original (swizzled)
            return
        }

        let proxy = _WKScriptMessageProxy(originalHandler: handler)
...
/// for CocoaDebug's own message names (log, error, warn, debug, info, networkCapture).
@objc class _WKWebViewScriptHandler: NSObject, WKScriptMessageHandler {
    ...
    private func handleNetworkCaptureMessage(_ message: WKScriptMessage) {
        ...
        DispatchQueue.main.async {
            ...
            // Notify UI
            NotificationCenter.default.post(name: NSNotification.Name("reloadHttp_CocoaDebug"), object: nil, userInfo: nil)
        }
    }
}
...
extension WKWebView {
    ...
    @objc func replaced_init(frame: CGRect, configuration: WKWebViewConfiguration) -> WKWebView {
        ...
        _OCLogHelper.shared.handleLog(
            ...
            line: 0,
            message: "----------------------------------- init -----------------------------------",
            color: _NetworkHelper.shared.mainColor ?? .green,
            type: .none
        )

        let handler = _WKWebViewScriptHandler()
