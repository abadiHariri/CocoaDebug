...
// MARK: - _QNSURLSessionDemuxTaskInfo

/// Holds per-task state: the delegate, the originating thread, and the run-loop modes.
private class _QNSURLSessionDemuxTaskInfo: NSObject {
...
/// these callbacks.
@objc class _QNSURLSessionDemux: NSObject {
    ...
    /// Maps `taskIdentifier` -> `_QNSURLSessionDemuxTaskInfo`. Access protected by `objc_sync`.
    private var taskInfoByTaskID: [Int: _QNSURLSessionDemuxTaskInfo] = [:]
    ...
    @objc func dataTask(with request: URLRequest,
        ...
        let taskInfo = _QNSURLSessionDemuxTaskInfo(task: task, delegate: delegate, modes: effectiveModes)
    ...
    private func taskInfo(for task: URLSessionTask) -> _QNSURLSessionDemuxTaskInfo? {
        objc_sync_enter(self)
        defer { objc_sync_exit(self) }
...
extension _QNSURLSessionDemux: URLSessionDataDelegate {
