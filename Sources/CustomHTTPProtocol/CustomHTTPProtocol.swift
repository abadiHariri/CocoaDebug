...
private class _CPBlockBox: NSObject {
    let block: () -> Void
...
// MARK: - _CustomHTTPProtocolDelegate

@objc protocol _CustomHTTPProtocolDelegate: NSObjectProtocol {

    @objc optional func customHTTPProtocol(_ protocol: _CustomHTTPProtocol,
...
// MARK: - _CustomHTTPProtocol

@objc class _CustomHTTPProtocol: URLProtocol {

    // MARK: Class-level delegate storage

    private static var sDelegate: _CustomHTTPProtocolDelegate?
    ...
    @objc static func getDelegate() -> _CustomHTTPProtocolDelegate? {
        var result: _CustomHTTPProtocolDelegate?
    ...
    @objc static func setDelegate(_ newValue: _CustomHTTPProtocolDelegate?) {
        objc_sync_enter(self)
        sDelegate = newValue
        objc_sync_exit(self)
    }
    ...
    private static var sharedDemuxInstance: _QNSURLSessionDemux?
    private static let demuxOnce: Void = {
        ...
        config.protocolClasses = [_CustomHTTPProtocol.self]
        sharedDemuxInstance = _QNSURLSessionDemux(configuration: config)
    }()

    @objc class func sharedDemux() -> _QNSURLSessionDemux {
        _ = demuxOnce
    ...
    /// (e.g. from `CocoaDebugSettings` or app delegate) because Swift
    ...
    @objc class func swizzleSessionConfiguration() {
        ...
        if !UserDefaults.standard.bool(forKey: "disableNetworkMonitoring_CocoaDebug") {
            configSwizzled = true
            ...
            let replacedDefault: @convention(block) (AnyObject) -> URLSessionConfiguration = { selfObj in
                ...
                _CustomHTTPProtocol.injectProtocol(into: config)
                return config
            }
            ...
            if let getterMethod = class_getInstanceMethod(configClass, protocolClassesSel) {
                ...
                let replacedGetter: @convention(block) (AnyObject) -> NSArray? = { configObj in
                    ...
                    let protoCls: AnyClass = _CustomHTTPProtocol.self
    ...
    /// Injects `_CustomHTTPProtocol` at the front of the given configuration's `protocolClasses`.
    ...
    override class func canInit(with request: URLRequest) -> Bool {
        ...
        if let onlyURLs = _NetworkHelper.shared.onlyURLs, onlyURLs.count > 0 {
            let url = request.url?.absoluteString.lowercased() ?? ""
            for onlyURL in onlyURLs {
                if url.range(of: onlyURL.lowercased()) != nil {
                    return true
                }
            }
            return false
    ...
    override func startLoading() {
        ...
        _CustomHTTPProtocol.setProperty(true,
                                        forKey: _CustomHTTPProtocol.kOurRecursiveRequestFlagProperty,
                                        in: recursiveRequest)
    ...
    override func stopLoading() {
        ...
        if let reqBody = self.capturedRequestBody, reqBody.count > 0 {
            let maxBodySize = _NetworkHelper.shared.maxRequestBodySize
    ...
    @objc private func onThreadPerformBlock(_ box: _CPBlockBox) {
        box.block()
    }
    ...
    private func mainThreadDidReceiveAuthenticationChallenge(_ challenge: URLAuthenticationChallenge,
        ...
        if self.pendingChallenge != nil {
            ...
            if !(strongDelegate?.responds(to: #selector(_CustomHTTPProtocolDelegate.customHTTPProtocol(_:canAuthenticateAgainstProtectionSpace:))) ?? false) {
                clientThreadCancelAuthenticationChallenge(challenge, completionHandler: completionHandler)
            } else {
    ...
    private func cancelPendingChallenge() {
        performOnThread(nil, modes: nil) {
            if self.pendingChallenge == nil {
                ...
                if strongDelegate?.responds(to: #selector(_CustomHTTPProtocolDelegate.customHTTPProtocol(_:didCancelAuthenticationChallenge:))) ?? false {
    ...
    @discardableResult
    private func handleError(_ error: Error?, model: _HttpModel) -> _HttpModel {
        if error == nil {
...
extension _CustomHTTPProtocol: URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession,
        ...
        if !self.responseTruncated {
            let maxSize = Int(_NetworkHelper.shared.maxResponseSize)
