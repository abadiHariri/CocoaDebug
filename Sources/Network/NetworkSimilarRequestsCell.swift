...
/// Tapping a card fires `onTap` with the corresponding `_HttpModel`.
final class NetworkSimilarRequestsCell: UITableViewCell {
    ...
    var onTap: ((_HttpModel) -> Void)?

    // MARK: - Private state

    private var models: [_HttpModel] = []
    ...
    func configure(with httpModels: [_HttpModel]) {
        models = httpModels
        stackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
    ...
    private func makeMiniCard(for model: _HttpModel, index: Int) -> UIView {
        let card = UIView()
